const reviews = [
  { name: "Niladri Kumar", text: "A big shout-out to Blue Ramp and all the young brains and brawn behind this amazing event. Keep your brains intact, stay creative, and continue doing such fantastic work. Wishing you all the best for future events and success." },
...
function createReviewCard(review) {
  return `
    <div class="card m-2 shadow-sm h-100">
      ${review.img ? `<img src="${review.img}" class="card-img-top" alt="${review.name}" style="object-fit: cover; height: 180px;">` : ''}
      <div class="card-body d-flex flex-column justify-content-between">
        <div class="review-text mb-2">${review.text}</div>
        <h6 class="card-subtitle text-muted mt-2">– ${review.name}</h6>
      </div>
    </div>
  `;
